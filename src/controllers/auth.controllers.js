...
const cretaeUser = async (req, res) => {
    try {
        ...
        try {
            await pusher.trigger("admin-customers", "new-customer", {
                _id: user._id,
            ...
            console.log(pushErr, "pusher trigger failed for new-customer");
        }

        const notifSettings = await NotificationSettings.findOne();
        const notifySignupsEnabled = notifSettings ? notifSettings.notifySignups : true;
        ...
        await createNotification({
            ...
            relatedId: user._id,
            notifyEnabled: notifySignupsEnabled,
        });
